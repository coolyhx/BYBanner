...
public class LoopViewPager extends ViewPager {
    ...
    private static final int AUTO_SCROLL_MESSAGE_ID = 0x1124;
    ...
    private Handler mAutoScrollHandler = new Handler(Looper.getMainLooper()){
        ...
        public void handleMessage(Message msg) {
            if(mIsAutoScroll){
                ...
                mAutoScrollHandler.removeMessages(AUTO_SCROLL_MESSAGE_ID);
            }
        }
    };

    public void setAutoScroll(boolean flag){
        this.mIsAutoScroll = flag;
        if(flag){
            goNext();
        }else{
            pauseAutoScroll();
    ...
    private void goNext(){
        mAutoScrollHandler.removeMessages(AUTO_SCROLL_MESSAGE_ID);
        Message msg = mAutoScrollHandler.obtainMessage(AUTO_SCROLL_MESSAGE_ID);
        mAutoScrollHandler.sendMessageDelayed(msg,DEFAULT_AUTO_SCROLL_INTERVAL);
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if(this.mIsAutoScroll && ev.getActionMasked() == MotionEvent.ACTION_DOWN){
            pauseAutoScroll();
        }
        if(this.mIsAutoScroll && ev.getActionMasked() == MotionEvent.ACTION_UP){
            goNext();
        }
